...
class FormController extends AbstractController
{
    ...
    public function filmAddAction(EntityManagerInterface $em, Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $em->persist($film);
            $em->flush();
            $this->addFlash('info','film valide');
        ...
        return $this->render('Sandbox/Form/film_add.html.twig', $argc);
    }
   #[Route(
        '/critique/add',
        name: '_critique_add',
    ...
    public function  critiqueAddAction(EntityManagerInterface $em, Request $request): Response
    {
       ...
       $form = $this->createForm(CritiqueType::class, $critique1,);
       $form->add('send', SubmitType::class, ['label' => 'add critique']);
       $form->handleRequest($request);
       if ($form->isSubmitted() && $form->isValid()) {
           $em->persist($critique1);
           $em->flush();
           $this->addFlash('info','critique valide');
           return $this->redirectToRoute('sandbox_doctrine_critique_view2', ['id' => $critique1->getFilm()->getId()]);
       }
       if ($form->isSubmitted())
           $this->addFlash('info', 'formulaire critique incorrect');
       $args = array(
           'myform' => $form,
       );
